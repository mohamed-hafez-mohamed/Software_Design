...
u8   MSTK_u8IsStkExpired(u8 * Copy_u8StkState)
{
    if(GET_BIT(STK->CTRL, COUNTFLAG))
    {
        *Copy_u8StkState = 1;
    }
    else
...
u8 MSTK_u8StartStkSingle_ms(u16 Copy_u16DelayMs)
{
...
u8 MSTK_u8StartStkPeriodic_ms(u16 Copy_u16DelayMs)
{
    u16 Local_u16Ticks = STK_u16ConfigIntervalMs();
    CLR_BIT(STK->CTRL, ENABLE);
    STK->VAL  = STK_CLR_REG;
...
u8   MSTK_u8StartStkSingle_us(u32 Copy_u32DelayUs)
{
...
u8   MSTK_u8StartStkPeriodic_us(u32 Copy_u32DelayUs)
{
    u8 Local_u8Ticks = STK_u8ConfigIntervalUs();
    CLR_BIT(STK->CTRL, ENABLE);
    STK->VAL  = STK_CLR_REG;
